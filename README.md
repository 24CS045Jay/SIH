...
Phase 3 adds demo-friendly JWT authentication under `/api/v1/auth`. The login screen loads seeded users from `/api/v1/auth/demo-users`; use the clearly labeled **DEMO LOGIN** picker with the password `demo-password`. In Vite development, `/api` is proxied to the backend on port 8000; in the container stack, Nginx proxies the same relative path to the backend service. If the backend is temporarily unavailable, the login screen now shows the HTTP reason and provides a retry control. The token embeds the user ID, name, email, role, department ID, and department name. The frontend stores the token only for the local demo session and provides a logout menu.
...
The production integration can call the built-in OpenAI-compatible proxy with `INTELLIGENCE_LLM_ENABLED=true` and the `gpt-5-mini` model. The request uses strict JSON Schema output and treats OCR text as untrusted data. For the synthetic hackathon demo, the default is deterministic, schema-validated extraction with automatic local embeddings and extractive RAG generation, so the portal remains runnable without external credentials; it covers circular classification, dates, departments, assets, locations, identifiers, monetary references, obligations, deadlines, summaries, priorities, reason codes, and routing recommendations.

If an optional OpenAI key is present but invalid or expired, the intelligence service falls back to deterministic extraction and the embedding service falls back to local TF-IDF vectors. OCR is not marked failed merely because an optional AI provider token is unavailable. Configure `EMBEDDING_PROVIDER=openai` or `RAG_GENERATION_PROVIDER=openai` only when valid credentials are intentionally required.
...
The complete demo stack is containerized in [`docker-compose.yml`](docker-compose.yml), with separate backend/worker and frontend Dockerfiles. Follow [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md) to bring PostgreSQL, Redis, FastAPI, Celery, and Nginx up from zero, run migrations and the synthetic corpus seed, check health, and execute the Phase 13 smoke tests. Never commit `.env` files or real credentials.
